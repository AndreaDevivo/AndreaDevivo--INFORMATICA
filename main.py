...
def crea_arma_per(giocatore: Giocatore) -> Arma:
    if giocatore.forza >= giocatore.destrezza:
...
def main():
    ...
    print(f"{g1.nome} equipaggia: {arma1}")
    print(f"{g2.nome} equipaggia: {arma2}\n")
    ...
    if g1.è_vivo() and not g2.è_vivo():
        print(f" {g1.nome} ha vinto il combattimento!")
    elif g2.è_vivo() and not g1.è_vivo():
        print(f" {g2.nome} ha vinto il combattimento!")
    else:
